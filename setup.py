...
# Get version without importing scandir because that will lock the .pyd file (if scandir is already installed) so it can't be overwritten during the install process
...
class BuildExt(base_build_ext):
    ...
    python_include_dir = '/usr/include/tirpc/python2.7/Python.h'
    ext = Extension('_scandir', ['_scandir.c'], optional=True, include_dirs=[python_include_dir])
